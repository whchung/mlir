...
public:
  ...
  void runOnModule() override {
    ...
    llvm::errs() << "MIOpenDriverPass runOnModule()\n";
...
//OwnedHSACO
//MIOpenDriverPass::emitModuleToHSACO(llvm::Module &llvmModule,
//                                        llvm::TargetMachine &targetMachine) {
//
...
//
//OwnedHSACO
//MIOpenDriverPass::emitModuleToHSACOForTesting(llvm::Module &llvmModule,
//                                                  FuncOp &function) {
//  const char data[] = "HSACO";
//  return std::make_unique<std::vector<char>>(data, data + sizeof(data) - 1);
//}
//
//OwnedHSACO MIOpenDriverPass::convertModuleToHSACO(llvm::Module &llvmModule,
//                                                      FuncOp &function) {
//
//  if (config.testMode) {
...
//}
//
//LogicalResult MIOpenDriverPass::translateGpuKernelToHSACOAnnotation(
...
LogicalResult MIOpenDriverPass::invokeMIOpenDriverPerFunction(
  ...
  //llvm::errs() << "MIOpenDriverPass::invokeMIOpenDriverPerFunction\n";

  function.walk([&](Operation *op) {
    if (auto constantOp = dyn_cast_or_null<ConstantOp>(op)) {
      llvm::errs() << "found target op, invoke MIOpenDriver:";
      //constantOp.dump();
      //llvm::errs() << "execute MIOpenDriver\n";
      ...
      //lldArgs[4] = llvm::StringRef(isabinPath.c_str());
      //lldArgs[6] = llvm::StringRef(hsacoPath.c_str());
      std::vector<llvm::StringRef> miopenDriverArgs{
      };
