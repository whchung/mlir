...
constexpr const char *kHSACOAnnotation = "amdgpu.hsaco";
constexpr const char *kHSACOGetterAnnotation = "amdgpu.hsacogetter";
constexpr const char *kHSACOGetterSuffix = "_hsaco";
...
private:
  ...
  FuncOp generateHSACOAccessor(Builder &builder, FuncOp &orig,
                               StringAttr blob) {
    Location loc = orig.getLoc();
    SmallString<128> nameBuffer(orig.getName());
    nameBuffer.append(kHSACOGetterSuffix);
    ...
    orig.setAttr(kHSACOGetterAnnotation, builder.getSymbolRefAttr(result));
    return result;
  }
...
public:
  ...
  void runOnModule() override {
    ...
    for (auto it = functions.begin(); it != functions.end();) {
      ...
      // of the accessor is after the kernel with hsaco iself.
      FuncOp orig = *it++;
      StringAttr hsacoBlob = orig.getAttrOfType<StringAttr>(kHSACOAnnotation);
      if (!hsacoBlob)
        continue;
      module.insert(it, generateHSACOAccessor(builder, orig, hsacoBlob));
    }
  }
...
ModulePassBase *createGenerateHSACOAccessorPass() {
  return new GpuGenerateHSACOAccessorsPass();
}
